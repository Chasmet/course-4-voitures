...
function getRoadData() {
  const topWidth = canvas.width * 0.40;
...
function maybeSpawnObstacle() {
  ...
  if (Math.random() < lvl.barrierRate) {
    ...
    obstacles.push({
      type: "barrier",
      lane,
      distance: cars[0].distance + rand(260, 520),
      w: 28,
      h: 18
    });
  }
  ...
  if (Math.random() < lvl.animalRate) {
    ...
    obstacles.push({
      type: "animal",
      lane,
      distance: cars[0].distance + rand(280, 540),
      w: 22,
      h: 14
    });
  }
...
function cleanupObstacles() {
  obstacles = obstacles.filter(o => o.distance > cars[0].distance - 200);
}

function obstacleScreenY(objDistance) {
  const player = cars[0];
  const delta = objDistance - player.distance;
  const y = player.y - delta * 0.42;
  return y;
}
...
function drawBackground() {
  ...
  for (let i = -1; i < 5; i++) {
    const x = i * 170 - ((worldTime * 10) % 170);
    ctx.beginPath();
...
function drawRoad() {
  ...
  for (let d = Math.floor(playerDistance / 80) * 80 - 240; d < playerDistance + 1200; d += 80) {
...
function updatePlayer() {
  ...
  const accelSpeed = 7.2;
  const boostExtra = 3.2;
  const coast = 0.12;
...
function updateAI() {
  ...
  for (let i = 1; i < cars.length; i++) {
    ...
    if (started && Math.random() < 0.006) {
      const dir = Math.random() < 0.5 ? -1 : 1;
      car.lane = Math.max(0, Math.min(3, car.lane + dir));
    }

    if (started && Math.random() < 0.003) {
      car.boost = 12;
    }
...
function updateCurve() {
  ...
  if (!started) return;
  curvePhase += 0.012;
  roadShift = Math.sin(curvePhase) * 35 * lvl.curveStrength;
}
...
function hitPlayerObstacle() {
  ...
  for (const o of obstacles) {
    ...
    const closeDistance = Math.abs(o.distance - player.distance) < 34;
...
function renderCars() {
  ...
  for (const car of cars) {
    ...
    if (y < 40 || y > canvas.height + 60) continue;
